...
public class MobileEngine {
	...
	public MobileLibrary objMobileFunc;
		...
		try {
			...
			try {
				...
				try {
					...
					MobileLibrary.staticCopyFile(src,
	...
	public void beforeTest(ITestContext context) throws IOException {
		...
		sysInfo.put("Time Zone", MobileLibrary.getCurrentTimeZone());
	...
	public void beforeClass(ITestContext context) throws IOException {
		try {
			...
			CommonVariables.setMobileActionLib();
			objMobileFunc = CommonVariables.getMobileActionLib();
			try {
		...
		///////////////////Venu
		/* Code to read the parameters from the XML test */
		try {
			if (context.getCurrentXmlTest().getTestParameters().size() > 0) {
				if (context.getCurrentXmlTest().getTestParameters().containsKey("udid")) {
					CommonVariables.UDID.set(context.getCurrentXmlTest().getParameter("udid").trim());
				} else {
					CommonVariables.UDID.set(context.getCurrentXmlTest().getParameter("udid").trim());
				}
			} else {
				CommonVariables.UDID.set(ConfigManager.getProperties().getProperty("udid").trim());
			}
		} catch (Exception e) {
			CommonVariables.IsCloudExecution.set(false);
		}
		...
		try {
			if (context.getCurrentXmlTest().getTestParameters().size() > 0) {
				if (context.getCurrentXmlTest().getTestParameters().containsKey("udid")) {
					CommonVariables.platformVersion.set(context.getCurrentXmlTest().getParameter("platformVersion").trim());
				} else {
					CommonVariables.platformVersion.set(context.getCurrentXmlTest().getParameter("platformVersion").trim());
				}
			} else {
				CommonVariables.platformVersion.set(ConfigManager.getProperties().getProperty("platformVersion").trim());
			}
		} catch (Exception e) {
			CommonVariables.IsCloudExecution.set(false);
		}
		...
		try {
			if (context.getCurrentXmlTest().getTestParameters().size() > 0) {
				if (context.getCurrentXmlTest().getTestParameters().containsKey("serverURL")) {
					CommonVariables.serverURL.set(context.getCurrentXmlTest().getParameter("serverURL").trim());
				} else {
					CommonVariables.serverURL.set(context.getCurrentXmlTest().getParameter("serverURL").trim());
				}
			} else {
				CommonVariables.serverURL.set(ConfigManager.getProperties().getProperty("serverURL").trim());
			}
		} catch (Exception e) {
			CommonVariables.IsCloudExecution.set(false);
		}
		...
		try {
			if (context.getCurrentXmlTest().getTestParameters().size() > 0) {
				if (context.getCurrentXmlTest().getTestParameters().containsKey("browserType")) {
					CommonVariables.browserName.set(context.getCurrentXmlTest().getParameter("browserType").trim());
				} else {
					CommonVariables.browserName.set(context.getCurrentXmlTest().getParameter("browserType").trim());
				}
			} else {
				CommonVariables.browserName.set(ConfigManager.getProperties().getProperty("browserType").trim());
			}
		} catch (Exception e) {
			CommonVariables.IsCloudExecution.set(false);
		}
		...
		if (CommonVariables.CommonDriver.get() == null) {
			try {
				objMobileFunc.locationServiceEnabled = false;
				WebDriver caseDriver = objMobileFunc.instantiateDriver(CommonVariables.PlatformName.get() + "-" + CommonVariables.DeviceName.get(), CommonVariables.UDID.get(), CommonVariables.platformVersion.get(), CommonVariables.browserName.get(), CommonVariables.serverURL.get());
				//objMobileFunc.setDriver((EventFiringWebDriver) caseDriver);
				objMobileFunc.setDriver((AppiumDriver) caseDriver);
				...
				/*if (ConfigManager.getProperties().getProperty("IsBrowserMobProxyRequire").equalsIgnoreCase("Yes")) {
					...
					objMobileFunc.OpenApplication(ConfigManager.getProperties().getProperty("url"));
				}*/
				if (ConfigManager.getProperties().getProperty("IsBrowserMobProxyRequire").equalsIgnoreCase("Yes")) {
					ConfigManager.getProperties().getProperty("urlSuppressCredential");
				}else if (ConfigManager.getProperties().getProperty("TestingType").equalsIgnoreCase("MobileWeb")) {
					objMobileFunc.OpenApplication(ConfigManager.getProperties().getProperty("url"));
				}else {
					objMobileFunc.OpenApplication(ConfigManager.getProperties().getProperty("url"));
				}
	...
	public void afterClass() {
		try {
			...
			/////Commented the below code as the mobile driver does not support getwindowhandle()
			//if (CommonVariables.getDriver() == null || CommonVariables.getDriver().getWindowHandle() == null) {
			if (CommonVariables.getDriver() == null) {
			} else {
				CommonVariables.getMobileActionLib().tearDown();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void beforeMethod(Method method) throws IOException {
		...
		if (CommonVariables.CommonDriver.get() == null) {
			objMobileFunc.locationServiceEnabled = false;
			WebDriver caseDriver = objMobileFunc
					.instantiateDriver(CommonVariables.PlatformName.get() + "-" + CommonVariables.DeviceName.get(), CommonVariables.UDID.get(), CommonVariables.platformVersion.get(), CommonVariables.browserName.get(), CommonVariables.serverURL.get());
			objMobileFunc.setDriver((EventFiringWebDriver) caseDriver);
			CommonVariables.setDriver(caseDriver);
//			if (ConfigManager.getProperties().getProperty("IsBrowserMobProxyRequire").equalsIgnoreCase("Yes")) {
//			ConfigManager.getProperties().getProperty("urlSuppressCredential");
...
//		}
	...
	public String getTestNGGroups(ITestContext context) {
		...
		groupName = MobileLibrary.ChangeStringFirstCharToCapital(groupName);
		return groupName;
	}
