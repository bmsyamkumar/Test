		...
		try {
			...
			if (!flag) {
				...
				Reporter.SuccessReport("Click", "Successfully clicked on "
						+ locatorName);
				logger.info("Successfully clicked on "
						+ locatorName);
			}
		}
		//Thread.sleep(2000);
...
//				
//				Thread.sleep(1000);
//			    new ProcessBuilder(new String[]{"adb", "-s", DeviceName, "shell", "input", "text", testdata})
...
//			}
			...
			if (!flag) {
				Reporter.failureReport("Type ",
						"Data typing action is not performed on " + locatorName
								+ " with data <b>" + testdata + "</b");
			} else if (b && flag) {
				Reporter.SuccessReport("Type ",
						"Data typing action is performed on " + locatorName
								+ " with data <b>" + testdata + "</b");
				logger.info("Data typing action is performed on " + locatorName
								+ " with data <b>" + testdata + "</b");
			}
		}
		return flag;
	...
	public static boolean doubleTapElement(By by,String locatorName) throws Throwable {
        boolean flag = false;
        try {
			...
			if (!flag) {
				Reporter.failureReport("DoubleClick", "Unable to doubleclick on "
						+ locatorName);
				return flag;
			} else if (b && flag) {
				Reporter.SuccessReport("DoubleClick", "Performed double click action on "
						+ locatorName);
				logger.info("Performed double click action on "
						+ locatorName);
			}
		}
		return flag;
	...
	public static boolean waitForElementPresent(By by, String locator)
		...
		try {
			...
			Assert.assertTrue(flag,"waitForElementPresent : Failed to locate element "+locator);
			...
			if (!flag) {
				Reporter.failureReport("WaitForElementPresent ",
						"Failed to locate element " + locator);
			} else if (b && flag) {
	...
	public static boolean selectBySendkeys(By locator, String value,
		...
		try {
			...
			if (!flag) {
				Reporter.failureReport("Select ", value	+ "is Not Select from the DropDown " + locatorName);
				// throw new ElementNotFoundException("", "", "");
...
//			}
	...
	public static void tapOn(String loginLogo) throws Throwable{
        ...
        WebElement el = driver.findElement(By.xpath("//*[@text='Login'][@resource-id='tv.hooq.android:id/title']"));
        ...
        Reporter.SuccessReport("TripleTap", "Succesfully triple tapped on Login title");
	...
	public static void swipeUpOrBottom(boolean bottomToTop) throws Throwable{
		...
		if(bottomToTop){
			...
			Reporter.SuccessReport("SwipeAction", "Swipe from bottom to top has performed");
		} else{
			((AndroidDriver)driver).swipe(startx, endy, startx, starty, 3000);
			Reporter.SuccessReport("SwipeAction", "Swipe from top to bottom has performed");
		}
	...
	public static MobileElement getMobileElement(By b){
	...
	public static String getPropertyValue(By locator, String propertyName, String locatorName)
			throws Throwable {
		String propVal = "";
		boolean flag = false;
		try {
			if (driver.findElement(locator).isDisplayed()) {
				propVal = driver.findElement(locator).getAttribute(propertyName);
				flag = true;
			}
		} catch (Exception e) {
			Assert.assertTrue(flag," Unable to get property '" + propertyName + "' value from " + locatorName);
			e.printStackTrace();
		} finally {
			if (!flag) {
				Reporter.failureReport("getPropertyValue ", " Unable to get property '" + propertyName + "' value from " + locatorName);
			} else if (b && flag) {
				Reporter.SuccessReport("getPropertyValue ", " Able to get value from " + locatorName);
			}
		}
		return propVal;
	}
	
	public static boolean scrollToExactText(final String text)
			throws Throwable {
		boolean flag = false;
		try {
			if(browser.toLowerCase().contains("iphone")){
				...
				AndroidDriver2.scrollToExact(text);
			}
			flag = true;
			return true;
		} catch (Exception e) {
	...
	public static void searchScreenForElement(By locator, int iIndex) throws Throwable
	{
...
//			if (driver.findElement(locator).size()>0)
//			exit;
//			}
//			//driver.findElement(By.name("YourText")).click();
	...
	public static void performSwipes(int iNumberOfSwipes) throws Throwable
	{
//		(AndroidDriver2).hideKeyboard();
		for(i=1;i<=iNumberOfSwipes;i++){
